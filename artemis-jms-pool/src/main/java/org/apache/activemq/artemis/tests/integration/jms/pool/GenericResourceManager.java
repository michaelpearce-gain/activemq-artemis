   ...
   public void recoverResource() {
      try {
         if (!Recovery.recover(this)) {
            ActiveMQJMSClientLogger.LOGGER.info("Resource manager is unrecoverable");
         }
      } catch (NoClassDefFoundError e) {
         ActiveMQJMSClientLogger.LOGGER.info("Resource manager is unrecoverable due to missing classes: " + e);
      } catch (Throwable e) {
         ActiveMQJMSClientLogger.LOGGER.warn("Error while recovering resource manager", e);
      }
   }
            ...
            rtxManager.registerNamedXAResourceFactory(new NamedXAResourceFactory() {
               ...
               public NamedXAResource getNamedXAResource() throws SystemException {
                  try {
                     ...
                     xaConnection.start();
                     ActiveMQJMSClientLogger.LOGGER.debug("new namedXAResource's connection: " + xaConnection);
                     ...
                     se.initCause(e);
                     ActiveMQJMSClientLogger.LOGGER.error(se.getLocalizedMessage(), se);
                     throw se;
                  }
               }
               ...
               public void returnNamedXAResource(NamedXAResource namedXaResource) {
                  if (namedXaResource instanceof ConnectionAndWrapperNamedXAResource) {
                     try {
                        ActiveMQJMSClientLogger.LOGGER.debug("closing returned namedXAResource's connection: " + ((ConnectionAndWrapperNamedXAResource) namedXaResource).connection);
                        ((ConnectionAndWrapperNamedXAResource) namedXaResource).connection.close();
                     } catch (Exception ignored) {
                        ActiveMQJMSClientLogger.LOGGER.debug("failed to close returned namedXAResource: " + namedXaResource, ignored);
                     }
                  }
               }
            });
