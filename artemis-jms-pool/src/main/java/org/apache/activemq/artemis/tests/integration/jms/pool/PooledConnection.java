   ...
   protected void cleanupConnectionTemporaryDestinations() {
      ...
      for (TemporaryQueue tempQueue : connTempQueues) {
         try {
            ...
            ActiveMQJMSClientLogger.LOGGER.info("failed to delete Temporary Queue \"" + tempQueue.toString() + "\" on closing pooled connection: " + ex.getMessage());
         }
      }
      ...
      for (TemporaryTopic tempTopic : connTempTopics) {
         try {
            ...
            ActiveMQJMSClientLogger.LOGGER.info("failed to delete Temporary Topic \"" + tempTopic.toString() + "\" on closing pooled connection: " + ex.getMessage());
         }
      }
   ...
   protected void cleanupAllLoanedSessions() {
      ...
      for (PooledSession session : loanedSessions) {
         try {
            ...
            ActiveMQJMSClientLogger.LOGGER.info("failed to close laoned Session \"" + session + "\" on closing pooled connection: " + ex.getMessage());
         }
      }
