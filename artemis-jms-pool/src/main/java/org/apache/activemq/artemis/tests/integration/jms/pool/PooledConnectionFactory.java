 ...
 * eviction thread may be configured using the {@link org.apache.activemq.artemis.tests.integration.jms.pool.PooledConnectionFactory#setTimeBetweenExpirationCheckMillis} method.  By
...
public class PooledConnectionFactory implements ConnectionFactory, QueueConnectionFactory, TopicConnectionFactory {
         ...
         this.connectionsPool = new GenericKeyedObjectPool<ConnectionKey, ConnectionPool>(
               new KeyedPooledObjectFactory<ConnectionKey, ConnectionPool>() {
                  ...
                  public PooledObject<ConnectionPool> makeObject(ConnectionKey connectionKey) throws Exception {
                     ...
                     ActiveMQJMSClientLogger.LOGGER.trace("Created new connection: " + connection);
                  ...
                  public void destroyObject(ConnectionKey connectionKey, PooledObject<ConnectionPool> pooledObject) throws Exception {
                     ...
                     try {
                        ActiveMQJMSClientLogger.LOGGER.trace("Destroying connection: " + connection);
                        connection.close();
                     } catch (Exception e) {
                        ActiveMQJMSClientLogger.LOGGER.warn("Close connection failed for connection: " + connection + ". This exception will be ignored.", e);
                     }
                  }
                  ...
                  public boolean validateObject(ConnectionKey connectionKey, PooledObject<ConnectionPool> pooledObject) {
                     ...
                     if (connection != null && connection.expiredCheck()) {
                        ActiveMQJMSClientLogger.LOGGER.trace("Connection has expired: {} and will be destroyed " + connection);
                        return false;
                     }
   ...
   public synchronized Connection createConnection(String userName, String password) throws JMSException {
      if (stopped.get()) {
         ActiveMQJMSClientLogger.LOGGER.debug("PooledConnectionFactory is stopped, skip create new connection.");
         return null;
      }
   ...
   public void start() {
      ActiveMQJMSClientLogger.LOGGER.debug("Staring the PooledConnectionFactory: create on start = " + isCreateConnectionOnStartup());
      stopped.set(false);
      if (isCreateConnectionOnStartup()) {
         try {
            ...
            ActiveMQJMSClientLogger.LOGGER.warn("Create pooled connection during start failed. This exception will be ignored.", e);
         }
      }
   }
      ...
      if (stopped.compareAndSet(false, true)) {
         try {
            if (connectionsPool != null) {
               ActiveMQJMSClientLogger.LOGGER.debug("Stopping the PooledConnectionFactory, number of connections in cache: "
   ...
   /**
    * @return true if a Connection is created immediately on a call to start.
    */
