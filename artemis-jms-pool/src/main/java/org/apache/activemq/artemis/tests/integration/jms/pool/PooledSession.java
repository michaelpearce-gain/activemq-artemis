         ...
         try {
            ...
            if (transactional && !isXa) {
               try {
                  ...
                  invalidate = true;
                  ActiveMQJMSClientLogger.LOGGER.warn("Caught exception trying rollback() when putting session back into the pool, will invalidate. " + e, e);
               }
            }
         } catch (JMSException ex) {
            invalidate = true;
            ActiveMQJMSClientLogger.LOGGER.warn("Caught exception trying close() when putting session back into the pool, will invalidate. " + ex, ex);
         } finally {
            consumers.clear();
            browsers.clear();
         ...
         if (invalidate) {
            ...
            if (sessionHolder != null) {
               try {
                  ...
                  ActiveMQJMSClientLogger.LOGGER.trace("Ignoring exception on close as discarding session: " + e1, e1);
               }
            }
            try {
               sessionPool.invalidateObject(key, sessionHolder);
            } catch (Exception e) {
               ActiveMQJMSClientLogger.LOGGER.trace("Ignoring exception on invalidateObject as discarding session: " + e, e);
            }
         } else {
            try {
