   ...
   public TransactionManager getTransactionManager() {
      if (transactionManager == null && tmFromJndi) {
         try {
            ...
            if (ActiveMQJMSClientLogger.LOGGER.isTraceEnabled()) {
               ActiveMQJMSClientLogger.LOGGER.trace("exception on tmFromJndi: " + getTmJndiName(), ignored);
            }
         }
      }
   ...
   private void configFromJndiConf(Object rootContextName) {
      if (rootContextName instanceof String) {
         ...
         try {
            ...
            if (ActiveMQJMSClientLogger.LOGGER.isTraceEnabled()) {
               ActiveMQJMSClientLogger.LOGGER.trace("exception on config from jndi: " + name, ignored);
            }
         }
      }
   }
