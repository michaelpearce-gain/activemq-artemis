...
public class JournalStorageManager extends AbstractJournalStorageManager {
   ...
   protected void init(Configuration config, IOCriticalErrorListener criticalErrorListener) {
      ...
      Journal localMessage = new JournalImpl(ioExecutors, fileSize, config.getJournalMinFiles(), config.getJournalPoolFiles(), config.getJournalCompactMinFiles(), config.getJournalCompactPercentage(), journalFF, "activemq-data", "amq", config.getJournalType() == JournalType.ASYNCIO ? config.getJournalMaxIO_AIO() : config.getJournalMaxIO_NIO(), 0);
