...
public class ArtemisContainerController {
   ...
   public void create(String containerQualifier, File configuration) {
      ArtemisDeployableContainer deployableContainer = getArtemisDeployableContainer(containerQualifier);
      deployableContainer.createBroker(configuration);
   }
   ...
   public BrokerFuture start(String containerQualifier) {
      ...
      return timeout -> {
         ...
         try (ServerLocator serverLocator = ActiveMQClient.createServerLocator(coreConnectUrl)) {
   ...
   public String getConnectHost(String containerQualifier) {
      ArtemisDeployableContainer deployableContainer = getArtemisDeployableContainer(containerQualifier);
      return deployableContainer.getConnectHost();
   }

   public String getConnectPort(String containerQualifier, String protocol) {
      ArtemisDeployableContainer deployableContainer = getArtemisDeployableContainer(containerQualifier);
      return deployableContainer.getConnectPort(protocol);
   }
   ...
   public String managementRequest(String container, String resource, String operationName, Object... args) {
      String coreConnectUrl = getCoreConnectUrl(container);
      try (ServerLocator locator = ActiveMQClient.createServerLocator(coreConnectUrl)) {
         ClientSessionFactory sessionFactory = locator.createSessionFactory();
         ClientSession session = sessionFactory.createSession();
         ClientRequestor requestor = new ClientRequestor(session, "activemq.management");
         ClientMessage message = session.createMessage(false);
